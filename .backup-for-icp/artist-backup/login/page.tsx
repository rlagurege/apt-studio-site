...
function LoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!slug || !password) {
      setError("Please select a staff member and enter a password.");
      setLoading(false);
      return;
    }
    
    try {
      console.log("[Login] Attempting sign in for:", slug);
      ...
      if (res?.error) {
        ...
        setError(`Login failed: ${res.error}. Please check your credentials.`);
        setLoading(false);
        return;
      }
      
      if (res?.ok && res?.url) {
        console.log("[Login] Sign in successful, redirecting to:", res.url);
        window.location.href = res.url;
      } else if (res?.ok) {
        // Sign in was successful but no URL, redirect to dashboard
            ...
            <div className="space-y-3">
              <PasskeyLogin
                ...
                onSuccess={() => {
                  ...
                  const finalUrl = slug === "tammi-gomula" ? "/admin/dashboard" : callbackUrl;
